...
int deform_conv_backward_parameters_cuda(
  ...
  for (int elt = 0; elt < batchSize / im2col_step; elt++) {
    ...
    columns =
        columns.reshape({group, columns.size(0) / group, columns.size(1)});
    gradWeight = gradWeight.reshape(
        {group, gradWeight.size(0) / group, gradWeight.size(1),
    ...
    gradWeight = gradWeight.reshape(
        {gradWeight.size(0) * gradWeight.size(1), gradWeight.size(2),
         gradWeight.size(3), gradWeight.size(4)});
  }
