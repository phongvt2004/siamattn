...
def build_opt_lr(model, current_epoch=0):
    ...
    # Multi-template fusion parameters
    trainable_params += [{'params': model.multi_template_fusion.parameters(),
    ...
    if cfg.MASK.MASK:
        ...
        trainable_params += [{'params': model.mask_head.parameters(),
                              'lr': cfg.TRAIN.BASE_LR}]
        trainable_params += [{'params': model.bbox_head.parameters(),
                              'lr': cfg.TRAIN.BASE_LR}]
...
def log_grads(model, tb_writer, tb_index):
    ...
    feature_norm, rpn_norm, fusion_norm = 0, 0, 0
    for k, g in grad.items():
        _norm = g.data.norm(2)
        weight = weights[k]
        w_norm = weight.norm(2)
        if 'feature' in k or 'backbone' in k:
    ...
    tot_norm = feature_norm + rpn_norm + fusion_norm
    tot_norm = tot_norm ** 0.5
    feature_norm = feature_norm ** 0.5
    rpn_norm = rpn_norm ** 0.5
    fusion_norm = fusion_norm ** 0.5

    tb_writer.add_scalar('grad/tot', tot_norm, tb_index)
    tb_writer.add_scalar('grad/feature', feature_norm, tb_index)
    tb_writer.add_scalar('grad/rpn', rpn_norm, tb_index)
    tb_writer.add_scalar('grad/fusion', fusion_norm, tb_index)
...
def main():
    ...
    # create model
    fusion_method = getattr(cfg.MODEL, 'FUSION_METHOD', 'attention') if hasattr(cfg, 'MODEL') else 'attention'
    model = CrossViewModelBuilder(fusion_method=fusion_method).cuda().train()
    ...
    if cfg.BACKBONE.PRETRAINED:
        ...
        logger.info("Loaded pretrained backbone from {}".format(backbone_path))
    ...
    # load pretrain
        ...
        logger.info("Loaded pretrained model from {}".format(cfg.TRAIN.PRETRAINED))
